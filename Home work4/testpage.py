...
class TestSearchLocators:
    ids = dict()
    with open(r"C:\Users\leito\PycharmProjects\pythonProject\Home work4\locators.yaml") as f:
...
# методы получения текста
    ...
    def get_text_blog(self):
        return self.get_text_from_element(TestSearchLocators.ids["LOCATOR_RESULT_LOGIN"], description="result word- "
                                                                                                      "blog")

    # функция проверки наличия созданоого поста- наличие названия поста
    def post(self):
        return self.get_text_from_element(TestSearchLocators.ids["LOCATOR_POST"], description="result- title of the "
                                                                                              "post")
